...
<div class="modal" id="chartModal">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <!-- Modal Header -->
            <div class="modal-header">
                <h4 class="modal-title">Tạo biểu đồ điểm danh</h4>
                <button type="button" class="close" data-dismiss="modal">&times;</button>
            </div>
            ...
            <!-- Modal footer -->
            <div class="modal-footer">
                <button type="button" name="create_chart" id="create_chart" class="btn btn-success btn-sm">Tạo biểu
                    đồ</button>
                <button type="button" class="btn btn-danger btn-sm" data-dismiss="modal">Đóng</button>
            </div>
...
$(document).ready(function() {
    ...
    $('#create_report').click(function() {
        ...
        if (error == 0) {
            ...
            window.open("report.php?action=attendance_report&grade_id=" + grade_id + "&from_date=" +
    ...
    $('#create_chart').click(function() {
        ...
        var error = 0;
        if (grade_id == '') {
            $('#error_chart_grade_id').text('Chọn lớp!');
            error++;
        } else {
            $('#error_chart_grade_id').text('');
        }
        if (attendance_date == '') {
            $('#error_attendance_date').text('Chọn ngày!');
            $error++;
        } else {
        ...
        if (error == 0) {
            ...
            $('#chart_grade_id').val('');
            $('#chartModal').modal('show');
            window.open("chart1.php?action=attendance_report&grade_id=" + grade_id + "&date=" +
                attendance_date);
        }
