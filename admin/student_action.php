...
if(isset($_POST["action"]))
{
	if($_POST["action"] == "fetch")
	{
		...
		foreach($result as $row)
		{
			...
			$sub_array[] = '<button type="button" name="edit_student" class="btn btn-primary btn-sm edit_student" id="'.$row["student_id"].'">Chỉnh sửa</button>';
			$sub_array[] = '<button type="button" name="delete_student" class="btn btn-danger btn-sm delete_student" id="'.$row["student_id"].'">Xóa</button>';
			$data[] = $sub_array;
		}
		$output = array(
	...
	if($_POST["action"] == 'Add' || $_POST["action"] == "Edit")
	{
		...
		{
			$error_student_name = 'Tên sinh viên không được để trống!';
			$error++;
		}
		else
		{
		...
		{
			$error_student_roll_number = 'Nhập mã sinh viên!';
			$error++;
		}
		else
		{
		...
		{
			$error_student_dob = 'Ngày sinh sinh viên không được để trống!';
			$error++;
		}
		else
		{
		...
		{
			$error_student_grade_id = "Thiếu mã lớp";
			$error++;
		}
		else
		{
		...
		else
		{
			if($_POST["action"] == 'Add')
			{
				...
				";
				$statement = $connect->prepare($query);
				$statement->execute(array(':student_roll_number' => $student_roll_number));
				...
				if($count > 0)
				{
					$output = array(
						'error'							=>	true,
						'error_student_roll_number'		=>	'Mã sinh viên đã tồn tại!'
					);
				}
				else
				{
