...
if(isset($_POST["action"]))
{
	if($_POST["action"] == "fetch")
	{
		...
		foreach($result as $row)
		{
			...
			$sub_array[] = '<button type="button" name="view_teacher" class="btn btn-info btn-sm view_teacher" id="'.$row["teacher_id"].'">Xem</button>';
			$sub_array[] = '<button type="button" name="edit_teacher" class="btn btn-primary btn-sm edit_teacher" id="'.$row["teacher_id"].'">Chỉnh sửa</button>';
			$sub_array[] = '<button type="button" name="delete_teacher" class="btn btn-danger btn-sm delete_teacher" id="'.$row["teacher_id"].'">Xóa</button>';
			$data[] = $sub_array;
		}
		$output = array(
	...
	if($_POST["action"] == 'Add' || $_POST["action"] == "Edit")
	{
		...
		if($_FILES["teacher_image"]["name"] != '')
		{
			...
			{
				$error_teacher_image = 'Định dạng ảnh không được hỗ trợ! (JPG hoặc PNG)';
				$error++;
			}
			else
			{
		...
		else
		{
			if($teacher_image == '')
			{
				$error_teacher_image = 'Không có ảnh!';
				$error++;
			}
		}
		if(empty($_POST["teacher_name"]))
		{
			$error_teacher_name = 'Tên giảng viên không được để trống!';
			$error++;
		}
		else
		{
		...
		{
			$error_teacher_address = 'Địa chỉ giảng viên không được để trống!';
			$error++;
		}
		else
		{
		...
		{
			$error_teacher_grade_id = "Nhập lớp!";
			$error++;
		}
		else
		{
		...
		{
			$error_teacher_doj = 'Ngày tham gia trường không được để trống!';
			$error++;
		}
		else
		{
			...
			if($_POST["action"] == 'Add')
			{
				...
				if($statement->execute($data))
				{
					...
					else
					{
						$output = array(
							'error'					=>	true,
							'error_teacher_emailid'	=>	'Email đã tồn tại!'
						);
					}
				}
			}
